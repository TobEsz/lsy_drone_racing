...
class RandTrajEnv(DroneEnv):
    ...
    def __init__(
        self,
        n_samples: int = 10,
        trajectory_time: float = 6.0,
        samples_dt: float = 0.1,
        *,
        num_envs: int = 1,
        max_episode_time: float = 6.0,
    ...
    def reward(self):
        ...
        # target waypoint
        goal = self.trajectories[
            jp.arange(self.trajectories.shape[0])[:, None], self.steps
        ][:, 0, :]

        # distance + normalized direction
        direction = goal - pos
        dist = jp.linalg.norm(direction, axis=-1)
        direction_norm = direction / (dist[..., None] + 1e-6)

        # 1) distance shaping
        distance_reward = jp.exp(-2.0 * dist)

        # 2) forward progress (only reward moving toward the goal)
        progress = jp.sum(vel * direction_norm, axis=-1)
        progress_reward = jp.clip(progress, 0.0, 5.0)

        # 3) velocity–trajectory alignment
        vel_norm = vel / (jp.linalg.norm(vel, axis=-1, keepdims=True) + 1e-6)
        alignment = jp.sum(vel_norm * direction_norm, axis=-1)
        alignment_reward = 0.1 * alignment

        # 4) off‑path penalty for drifting too far
        off_path_penalty = jp.where(dist > 0.1, -(dist - 0.1), 0.0)
        ...
        # crash penalty
        reward = jp.where(self.terminated(), -1.0, reward)
